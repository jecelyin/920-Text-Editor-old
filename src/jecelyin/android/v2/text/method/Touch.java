...
import jecelyin.android.v2.text.Layout.Alignment;
...
//            right = (int) Math.max(right, layout.getLineRight(i)); //jec-
            right = (int) Math.max(right, layout.getLineRight(i)+sLineNumberWidth);
    ...
    public static int getMaxScrollX(TextView widget, Layout layout, int y) {
        ...
        for (int i = top; i <= bottom; i++) {
            left = (int) Math.min(left, layout.getLineLeft(i));
//            right = (int) Math.max(right, layout.getLineRight(i)); //jec-
            right = (int) Math.max(right, layout.getLineRight(i)+sLineNumberWidth); //jec+
        }
