...
//            MetaKeyKeyListener.resetLockedMeta(buffer); //jec-
            resetLockedMeta(buffer); //jec+
            return true;
        }
...
//            MetaKeyKeyListener.resetLockedMeta(buffer); //jec-
    ...
    public boolean onTouchEvent(TextView widget, Spannable buffer, MotionEvent event) {
        ...
        int x = (int) event.getX() - sLineNumberWidth; //jec+

        if (widget.isFocused() && !widget.didTouchFocusSelect()) {
            if (event.getAction() == MotionEvent.ACTION_DOWN) {
              boolean cap = isCap(buffer);
              if (cap) {
//                  int offset = widget.getOffset((int) event.getX(), (int) event.getY()); //jec-
                  int offset = widget.getOffset(x, (int) event.getY()); //jec+                 
