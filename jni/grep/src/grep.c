...
#if !ENABLE_EXTRA_COMPAT
#define print_line(line, line_len, linenum, offset) \
	print_line(line, linenum, offset)
#endif
static void print_line(const char *line, size_t line_len, int linenum, long offset)
{
	LOGV("print_line： %s[%d]Offset: %d",cur_file, linenum, offset);
#if ENABLE_FEATURE_GREP_CONTEXT
...
#endif
	add_match_result(cur_file, linenum, line, offset);
/*
	if (print_filename)
 ...
 do_found:
			...
			if (option_mask32 & (OPT_q|OPT_l|OPT_L)) {
				...
				if (PRINT_FILES_WITH_MATCHES) {
					//puts(cur_file);
				    add_match_result(cur_file, 0, "", offset);
			...
			if (PRINT_MATCH_COUNTS == 0) {
				...
				if (lines_before && before_buf[prevpos] != NULL) {
					...
					while (before_buf[idx] != NULL) {
						print_line(before_buf[idx], before_buf_size[idx], first_buf_entry_line_num, offset);
						free(before_buf[idx]);
						before_buf[idx] = NULL;
						idx = (idx + 1) % lines_before;
						first_buf_entry_line_num++;
				...
				if (option_mask32 & OPT_o) {
					if (FGREP_FLAG) {
						...
						if (found)
							print_line(gl->pattern, strlen(gl->pattern), linenum, offset);
					} else while (1) {
						...
						if (len != 0)
							print_line(line + start, len, linenum, offset);
						if (old == '\0')
							break;
						line[end] = old;
						if (len == 0)
					...
					print_line(line, line_len, linenum, offset);
				}
			}
		}
#if ENABLE_FEATURE_GREP_CONTEXT
			...
			if (print_n_lines_after) {
				print_line(line, strlen(line), linenum, offset);
				print_n_lines_after--;
...
static char *add_grep_list_data(const char *pattern, int flg_used_mem)
#else
#define new_grep_list_data(p, m) add_grep_list_data(p)
static char *add_grep_list_data(const char *pattern)
#endif
{
			...
			int depth UNUSED_PARAM)
{
	...
	if (file == NULL) {
		EMSG2("file_action_grep err: %s", filename);
		open_errors = 1;
		return 0;
	}
	cur_file = filename;
...
int grep_main(int argc, const char **argv)
{
	...
	if (ENABLE_FEATURE_GREP_EGREP_ALIAS
	 && ((applet_name && applet_name[0] == 'e') || (option_mask32 & OPT_E))
	) {
